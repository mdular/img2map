...
module.exports = {
    ...
    fromUrl : function (url, callback, forceUpdate) {
        ...
        proto.get(url, (res) => {
            ...
            res.on('end', () => {
                ...
                // Custom lib way
                // TODO: further optimize gps exif resolving speed
                // TODO: no need to download the entire file
                var result = this.getCoordsFromBuffer(buf);
                this.mem[url] = result;
                callback(this.formatGeoPoint(result));
            })
        }).on('error', (evt) => {
    ...
    getCoordsFromBuffer: function (buffer) {
        var exif = Exif.fromBuffer(buffer);
        ...
        var gps = Exif.reduceTags(exif.GPS);
        // console.log(gps);

        if (typeof gps.GPSLongitude === 'undefined'
          || typeof gps.GPSLongitudeRef === 'undefined'
          || typeof gps.GPSLatitude === 'undefined'
          || typeof gps.GPSLatitudeRef === 'undefined'
        ) {
